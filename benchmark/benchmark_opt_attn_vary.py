...
def run(
    ...
    # Kernel flavor for the replaced linears: must match the smem opt-in below,
    # since each kernel needs its own cudaFuncSetAttribute for >48KB smem.
    ...
    if offload_ratio > 0:
        ...
        smem_size = get_smem_size(multicast=multicast_kernel)
        if multicast_kernel:
            if offload_ratio == 1.0:
                runtime.set_smem_size(smem_size, 2)  # GEMV_MULTICAST
                runtime.set_smem_size(smem_size, 4)  # GEMV_MULTICAST_PREFILL
            else:
                runtime.set_smem_size(smem_size, 3)  # GEMV_HORIZONTAL_MULTICAST
                runtime.set_smem_size(smem_size, 5)  # GEMV_HORIZONTAL_MULTICAST_PREFILL
        else:
            # GEMV (offload==1.0) / GEMV_HORIZONTAL (partial offload)
    ...
    if offload_ratio == 0:
        ...
        model = load_replaced_opt_for_causal_lm(
                    ...
                    LinearReplacementConfig(
                        ...
                        device="cpu",
                        dtype=dtype,
                        multicast_kernel=multicast_kernel,
                        drop_bias=drop_bias,
                    ),
                    config=cfg,
                    torch_dtype=dtype,
    ...
    # ---- fake prefill: 不跑 prefill，直接造一个 next_token ----
    torch.cuda.empty_cache()
    print_cuda_mem("before decode")

    vocab_size = model.config.vocab_size
    fake_prefill_logits = torch.randn((B, 1, vocab_size), device=device, dtype=dtype)
    next_token = torch.argmax(fake_prefill_logits[:, -1, :], dim=-1)
    del fake_prefill_logits
    out_ids[:, L] = next_token

    # 可选：提前把 StaticCache 建出来，否则显存会在第一次 warmup 里才分配，
    # 内存打印看起来会错位。host_size == B 时不要调（batch=0 会让 get_seq_length 越界）。
    if B - host_cache_size > 0:
        n_kv = getattr(model.config, "num_key_value_heads", model.config.num_attention_heads)
        gpu_cache.early_initialization(
            batch_size=B - host_cache_size,
            num_heads=n_kv,
            head_dim=model.config.hidden_size // model.config.num_attention_heads,
            dtype=dtype,
            device=torch.device(device),
        )
    print_cuda_mem("after fake prefill")
