...
def build_horizontal_config(offload_ratio: float, offload_m: int = 256):
    horizontal_config = {"GH200": {}}
    x = int(round(offload_ratio * 16, 0))
    print(f"!! debug: offload_ratio={offload_ratio}, x={x}")
...
def _make_cache(config: LlamaConfig, max_cache_len: int, offload_ratio: float) -> StaticCache:
...
def run(
    ...
    output_file: str = f"{ROOT_DIR}/eval/result/SplitKernel_mix.csv",
