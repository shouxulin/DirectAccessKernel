...
void check_query(const torch::Tensor& query) {
...
void check_kv_pair(
    ...
    bool expect_cuda,
    const char* label) {
  TORCH_CHECK(key.is_cuda() == expect_cuda, label, " key device is not expected");
  TORCH_CHECK(value.is_cuda() == expect_cuda, label, " value device is not expected");
  TORCH_CHECK(key.scalar_type() == query.scalar_type(), label, " key dtype must match query dtype");
  TORCH_CHECK(value.scalar_type() == query.scalar_type(), label, " value dtype must match query dtype");
  TORCH_CHECK(key.dim() == 4, label, " key must have shape [B, H, S, D]");
  TORCH_CHECK(value.dim() == 4, label, " value must have shape [B, H, S, D]");
  TORCH_CHECK(key.sizes() == value.sizes(), "key and value shapes must match");
  TORCH_CHECK(key.size(0) == expected_batch, label, " key batch does not match expected split batch size");
  TORCH_CHECK(key.size(1) == query.size(1), label, " key heads must match query heads");
  TORCH_CHECK(key.size(2) == expected_seq_len, label, " key seq_len must match expected seq_len");
  TORCH_CHECK(key.size(3) == query.size(3), label, " key head_dim must match query head_dim");
  TORCH_CHECK(key.stride(3) == 1, label, " key head_dim stride must be 1");
  TORCH_CHECK(value.stride(3) == 1, label, " value head_dim stride must be 1");
}
...
void check_static_cache_fast_path(
  ...
  void check_static_cache_tma_path(const torch::Tensor& key, const torch::Tensor& value, const char* label) {
...
torch::Tensor decode(
    ...
    torch::Tensor key,
    torch::Tensor value,
    std::optional<torch::Tensor> key_host,
    std::optional<torch::Tensor> value_host,
    int64_t host_size,
  ...
  const int64_t total_batch = query.size(0);
  TORCH_CHECK(host_size >= 0 && host_size <= total_batch,
              "host_size must be in [0, batch_size], got ",
              host_size,
              " for batch_size=",
              total_batch);
  const int64_t gpu_batch = total_batch - host_size;

  TORCH_CHECK(key.dim() == 4 && value.dim() == 4, "key/value must have shape [B, H, S, D]");
  TORCH_CHECK(key.size(0) == gpu_batch, "key/value GPU batch must equal batch_size - host_size");
  const int64_t key_seq_len = key.size(2);
  check_kv_pair(query, key, value, gpu_batch, key_seq_len, true, "gpu");
  check_static_cache_fast_path(query, key, value, "gpu", split_size);
  if constexpr (opt_attention::kUseTensorTma) {
    check_static_cache_tma_path(key, value, "gpu");
  }

  bool has_host_kv = host_size > 0;
  torch::Tensor key_host_tensor;
  torch::Tensor value_host_tensor;
  if (has_host_kv) {
    TORCH_CHECK(key_host.has_value() && key_host->defined(), "key_host is required when host_size > 0");
    TORCH_CHECK(value_host.has_value() && value_host->defined(), "value_host is required when host_size > 0");
    key_host_tensor = *key_host;
    value_host_tensor = *value_host;
    TORCH_CHECK(!key_host_tensor.is_cuda(), "key_host must be host memory");
    TORCH_CHECK(!value_host_tensor.is_cuda(), "value_host must be host memory");
    check_kv_pair(query, key_host_tensor, value_host_tensor, host_size, key_seq_len, false, "host");
    check_static_cache_fast_path(query, key_host_tensor, value_host_tensor, "host", split_size);
    if constexpr (opt_attention::kUseTensorTma) {
      check_static_cache_tma_path(key_host_tensor, value_host_tensor, "host");
    }
  }
  ...
  params.key_host = has_host_kv ? key_host_tensor.data_ptr() : nullptr;
  params.value_host = has_host_kv ? value_host_tensor.data_ptr() : nullptr;
  ...
  params.tma_descs_gpu = nullptr;
  params.tma_descs_host = nullptr;
  params.batch_size = static_cast<int>(query.size(0));
  params.host_size = static_cast<int>(host_size);
  params.num_heads = static_cast<int>(query.size(1));
  params.key_seq_len = static_cast<int>(key_seq_len);
  ...
  if constexpr (opt_attention::kUseTensorTma) {
    ...
    std::array<CUtensorMap, 4> host_descs{};
    size_t desc_count = 0;
    size_t gpu_desc_offset = 0;
    size_t host_desc_offset = 0;
    if (gpu_batch > 0) {
      gpu_desc_offset = desc_count;
      encode_static_cache_tma_desc(&host_descs[desc_count + 0], key);
      encode_static_cache_tma_desc(&host_descs[desc_count + 1], value);
      desc_count += 2;
    }
    if (has_host_kv) {
      host_desc_offset = desc_count;
      encode_static_cache_tma_desc(&host_descs[desc_count + 0], key_host_tensor);
      encode_static_cache_tma_desc(&host_descs[desc_count + 1], value_host_tensor);
      desc_count += 2;
    }
    TORCH_CHECK(desc_count > 0, "at least one KV source must be available");

    const auto desc_align = static_cast<uintptr_t>(alignof(CUtensorMap));
    tma_storage = torch::empty(
        {static_cast<int64_t>(desc_count * sizeof(CUtensorMap) + desc_align)},
    ...
    auto* desc_ptr = reinterpret_cast<const CUtensorMap*>(aligned_ptr);
    if (gpu_batch > 0) {
      params.tma_descs_gpu = desc_ptr + gpu_desc_offset;
    }
    if (has_host_kv) {
      params.tma_descs_host = desc_ptr + host_desc_offset;
    }
    C10_CUDA_CHECK(cudaMemcpyAsync(
        reinterpret_cast<void*>(aligned_ptr),
        host_descs.data(),
        desc_count * sizeof(CUtensorMap),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def(
      ...
      py::arg("key"),
      py::arg("value"),
      py::arg("key_host") = std::nullopt,
      py::arg("value_host") = std::nullopt,
      py::arg("host_size") = 0,
      py::arg("attention_mask") = std::nullopt,
      py::arg("scaling") = 1.0,
      py::arg("split_size") = 256,
      "OPT StaticCache decode attention");
