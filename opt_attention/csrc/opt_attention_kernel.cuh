...
template <typename scalar_t>
__device__ __forceinline__ const scalar_t* key_ptr_from_base(
...
template <typename scalar_t>
__device__ __forceinline__ const scalar_t* value_ptr_from_base(
    const OptAttentionParams& params,
    const scalar_t* base,
    int batch,
    int head,
...
__device__ __forceinline__ void producer_kv_tma_tile(
  ...
  const int flat_bhs_start = ((source_batch * params.num_heads + head) * params.key_seq_len + block_start);
  ...
  tma_load_2d(&smem.k[stage][0], &tma_descs[0], smem.barriers.kv_fill[stage], 0, flat_bhs_start);
  tma_load_2d(&smem.v[stage][0], &tma_descs[1], smem.barriers.kv_fill[stage], 0, flat_bhs_start);
}
...
__device__ void producer_kv(
  ...
  if constexpr (UseTma) {
    if (lane == 0) {
      tma_fence_acquire(&tma_descs[0]);
      tma_fence_acquire(&tma_descs[1]);
    }
  }
  ...
  for (int block = 0; block < num_blocks; ++block) {
    ...
    if constexpr (UseTma) {
      producer_kv_tma_tile(params, tma_descs, smem, block_start, stage, source_batch, head, lane);
    } else {
      const scalar_t* k0 = key_ptr_from_base<scalar_t>(params, key_base, source_batch, head, block_start);
      const scalar_t* v0 = value_ptr_from_base<scalar_t>(params, value_base, source_batch, head, block_start);
...
__global__ __launch_bounds__(kThreads, 1) void opt_attention_decode_kernel(OptAttentionParams params) {
  ...
  const bool use_host_source = batch < params.host_size;
  const int source_batch = use_host_source ? batch : (batch - params.host_size);
  ...
  if (threadIdx.x < kComputeThreads) {
    ...
    const auto* key_base = use_host_source
        ? static_cast<const scalar_t*>(params.key_host)
        : static_cast<const scalar_t*>(params.key);
    const auto* value_base = use_host_source
        ? static_cast<const scalar_t*>(params.value_host)
        : static_cast<const scalar_t*>(params.value);
    const auto* tma_descs = use_host_source ? params.tma_descs_host : params.tma_descs_gpu;
    producer_kv<scalar_t, UseTma>(
        params,
        smem,
        kv_start,
        kv_end,
